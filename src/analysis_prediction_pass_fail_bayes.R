...
nb_multiple_runs <- function(train_fraction,n){
  ...
  for (i in 1:n){
    ...
    nb_model <- naiveBayes(PASS~.,data = data)
    nb_test_predict <- predict(nb_model,data_test[,-1])
    fraction_correct[i] <- mean(nb_test_predict==data_test$PASS)
    true_positives <- 0
    false_positives <- 0
    false_negatives <- 0
    for (j in 1:length(nb_test_predict)) {
      if (nb_test_predict[j] && data_test$PASS[j]) { # true positives
        true_positives <- true_positives + 1
      }
      if (nb_test_predict[j] && !data_test$PASS[j]) { # false positives
        false_positives <- false_positives + 1
      }
      if (!nb_test_predict[j] && data_test$PASS[j]) { # false negative
...
naive_bayes <- function() {
  ...
  PASS <- data$PASS
  data <- cbind(PASS, data[,2:27], data[,43:50], data[,52:87])
  
  # train: 70%, 100 iterations
  results <- nb_multiple_runs(0.7, 100)
  # summary of results
  summary(results)
  apply(results, 2, sd)
